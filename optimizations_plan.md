...
## Execution log (orchestrator state)

- **2026-07-03:** A.1, B.1, B.2, C.1, C.2, D.2 implemented on `staging` (uncommitted). B/C/D were delegated to subagents and reviewed by the orchestrator; fixes applied during review: restored the shared `_TRANSFER_CONFIG`/`_r2_client` import in `backup_database_to_r2.py` (agent had duplicated them), swapped stderr-drain before `proc.wait()` (pipe-deadlock safety), and added a prod-connectivity probe to the mirror script before the schema wipe (streaming reordered wipe-before-dump). Verified: py_compile + `bash -n` clean, `manage.py check` clean, 32/32 tests green, B.2 old-vs-new mask pixel equivalence script passed on a synthetic class map.
- **Still pending (human, Railway console):** A.2 `MALLOC_ARENA_MAX=2`, A.3 worker-count experiment, D.1 check whether prod sets `CACHE_URL`/`REDIS_URL`.
- **Gated (do not do yet):** A.4 raise max-requests (needs Phase B deployed + a week of memory graph), B.3 subprocess isolation (only if RSS still ratchets after B.1/B.2), Phase E measurements.
- **Manual verification pending:** one real mask generation on staging comparing the output PNG pixel-diff against a pre-change mask (the synthetic equivalence check covers the vis pipeline, not the ONNX float branch).
...
- [x] Done — `--workers 2` set in start.sh (2026-07-03, user is cost-sensitive; app is a small-team tool)

Under ASGI, Django sync views serialize onto one thread per worker, so 2 workers ≈ 2 concurrent sync requests. Rollback criterion: if pages feel sluggish when several users are active simultaneously, bump to 3 — do not go back to 4 without evidence.
...
- [x] Done — user confirmed (2026-07-03) neither `CACHE_URL` nor `REDIS_URL` is set anywhere; no Redis service exists or is wanted (no paid add-ons). Prod will use the DatabaseCache fallback from D.2 — zero extra cost.
